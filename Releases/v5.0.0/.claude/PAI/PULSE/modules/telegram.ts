...
function log(level: "info" | "warn" | "error", msg: string, data?: unknown) {
  console.log(JSON.stringify({ ts: new Date().toISOString(), level, component: "telegram", msg, ...(data ? { data } : {}) }))
}
...
export async function startTelegram(config: TelegramConfig): Promise<void> {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      ...
      if (history.length > 0) {
        const historyText = history.slice(-10).map(m => `${m.role === "user" ? principalName : daName}: ${m.content}`).join("\n")
...
- When doing tasks, do them and confirm briefly what you did`,
      ...
      try {
        for await (const message of conversation) {
          ...
          if (msg.type === "assistant" && Array.isArray(msg.message?.content)) {
            for (const block of msg.message.content) if (block.type === "text" && block.text && !fullText) fullText = block.text
          }

          if (msg.type === "result") {
            if (msg.subtype === "success" && msg.result) fullText = msg.result
            if (msg.session_id) lastSessionId = msg.session_id
            log("info", "SDK session complete", { durationMs: Date.now() - startTime, numTurns: msg.num_turns, cost: msg.total_cost_usd, sessionId: lastSessionId })
          }
  ...
  await bot.start({ onStart: (info) => log("info", `Bot started: @${info.username}`, { botId: info.id }) })
}
...
export function telegramHealth(): {
  ...
  if (!bot) {
    return { status: activeConfig?.enabled === false ? "disabled" : "stopped", uptime_ms: 0, messages_received: 0, messages_responded: 0, processing: false }
  }

  return { status: "running", uptime_ms: Date.now() - startedAt, messages_received: messagesReceived, messages_responded: messagesResponded, processing, last_session_id: lastSessionId }
}
